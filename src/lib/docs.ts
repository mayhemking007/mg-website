...
const installCode = `npm install memo-grafter`;

const initCode = `npx memo-grafter init`;

const migrateCode = `npx memo-grafter migrate`;

const studioCode = `npx memo-grafter studio`;

const quickStartCode = `${installCode}
${initCode}
${migrateCode}`;

const minimalAgentCode = `import "dotenv/config";
...
const flowCode = `invoke() -> responds now
...
export const docPages = new Map(docsPages.map((page) => [page.slug, page]));
